 ...
 * 页面骨架
 */
define("common/base/component/imobilebone/imobilebone", function(require, exports, module) {
	// 需要使用扩展jquery方法
	...
	IMobilebone.footerElement = $("body>[data-role='footer']");

	// 使用手势
	//	IMobilebone.useSign = true;

	IMobilebone.initialize = function() {
	...
	*/
	IMobilebone.reset = function() {
		...
		this.pageElements.each(function(index) {
			...
			// 如果页面有页脚 则加上页脚的样式
			if(_this.footerElement.length > 0)	
				$(this).addClass('page-footer');
		});
	};
	...
	IMobilebone.swipe = function() {
		...
		SignEvent.swipe(document.body, function(e, p1, p2) {
			...
			if (_this.pageElements.first().data("translate").real > -window.innerWidth * 0.3 || _this.pageElements.last().data("translate").real < window.innerWidth * 0.3) {
				_this.pageElements.each(function() {
