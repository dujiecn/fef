(function() {
	...
	*/
		...
		for (var i = 0; i < dataArray.length; i++) {
			...
			if (serie.text != false) {
				this.drawText(context, centerX + radiusOut * Math.cos(textDegree + dataArray[i].radian * 0.5),
					...
					centerX + (radiusOut + 20) * Math.cos(textDegree + dataArray[i].radian * 0.5),
					centerY + (radiusOut + 20) * Math.sin(textDegree + dataArray[i].radian * 0.5), textDegree + dataArray[i].radian * 0.5, dataArray[i].name, dataArray[i].checked);
			}
	...
	// 画出线和各个扇形标题
	PieSerie.prototype.drawText = function(context, moveToX, moveToY, lineToX, lineToY, degree, textContext, checked) {
		var textX = 0,
			textY = lineToY;
		context.save();
		context.beginPath();
